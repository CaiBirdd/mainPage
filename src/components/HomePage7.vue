...
<template>
  <!-- 页面外层容器 -->
  <div class="page7-wrapper">
    <section class="page7">
      <!-- 主标题：Who Benefits from Synapnote? -->
      <h2 class="page7__title">
        Who <span class="page7__title-highlight">Benefits</span> from Synapnote?
      </h2>
      <!-- 副标题：描述文字 -->
      <p class="page7__subtitle">
        Professionals across industries are transforming how they capture and use knowledge
      </p>

      <!-- 轮播容器 -->
      <div class="page7__carousel-container">
        <!-- 轮播包装器：用于居中和对齐 -->
        <div class="page7__carousel-wrapper">
          <!-- 轮播主体：包含所有卡片 -->
          <div class="page7__carousel">
            <!-- 遍历渲染所有卡片 -->
            <div v-for="(card, index) in cards" :key="card.id" class="page7__card" :style="{
              ...getCardStyle(index), // 动态计算卡片位置和样式
              borderColor: card.borderColor // 设置卡片边框颜色
            }">
              <!-- 卡片背景图片：铺满整个卡片 -->
              <div class="page7__card-image" :style="{ backgroundImage: `url(${card.image})` }">
              </div>
              <!-- 卡片文字遮罩层：底部渐变遮罩，用于显示白色文字 -->
              <div class="page7__card-overlay">
                <h3 class="page7__card-title">{{ card.title }}</h3>
                <p class="page7__card-description">{{ card.description }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- 导航按钮组：左右切换箭头 -->
        <div class="page7__navigation">
          <!-- 上一张按钮 -->
          <button class="page7__nav-btn page7__nav-btn--prev" @click="goToPrev" aria-label="Previous">
          ...
          <!-- 下一张按钮 -->
          <button class="page7__nav-btn page7__nav-btn--next" @click="goToNext" aria-label="Next">
            <svg width="30" height="30" viewBox="0 0 30 30" fill="none">
        ...
        <!-- 进度条：显示当前轮播进度 -->
        <div class="page7__progress">
          <div class="page7__progress-bar" :style="{ width: `${progress}%` }">
        </div>
        </div>
      </div>
    </section>
